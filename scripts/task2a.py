...
def main():
    ...
    tolerance = 0.1 # controlling the error level in the manipluator end effector positions
    ssf = 1/1.0 # speed scaling factor
    ...
    def servo_move_1():
        ...
        try:
            ...
            node_2.get_logger().info(f'Could not transform base_link to obj_3: {e}')
            return
        try:
        ...
        err_y = np.abs(0.2 - y_pose)
        err_z = np.abs(0.65 - z_pose)
        ...
        if(is_task1_completed == False):
            ...
            if(err_z > tolerance):
                ...
                moveit2_servo(linear=(0.0, 0.2*ssf, 0.0), angular=(0.0, 0.0, 0.0))
            # moving to pickup point-1 
            elif(err_x > 0.0001):
                moveit2_servo(linear=(0.3*ssf, 0.0, 0.0), angular=(0.0, 0.0, 0.0))
            elif(err_x <= tolerance and err_y <= tolerance and err_z <= 0.001):
